...
module Nat_peano : NAT = struct
  ...
  let rec to_int x = 
    match x with
    | Zero -> 0
    | Succ x' ->  Int.add 1 (to_int x')
  let rec of_int x = 
    match x with 
    | 0 -> Zero
    | x' when x'> 0 -> Succ (of_int (Int.sub x' 1))
    | _ -> Zero
